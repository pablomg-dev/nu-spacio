        ...
        <header className="w-full bg-white shadow-md fixed top-0 left-0 z-50">
            <div className="max-w-6xl mx-auto flex items-center justify-between px-4 py-3 md:py-4">
                ...
                <nav className="hidden md:flex gap-8">
                    {navLinks.map(link => (
                        <a key={link.href} href={link.href} className="text-blue-800 font-medium hover:text-blue-600 transition">{link.name}</a>
                    ))}
                </nav>
            ...
            <nav className={`md:hidden fixed top-0 right-0 h-full w-64 bg-white shadow-lg z-50 transform transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <ul className="flex flex-col gap-6 mt-20 px-8">
                    {navLinks.map(link => (
                        <li key={link.href}><a href={link.href} className="text-blue-800 text-lg font-medium hover:text-blue-600 transition" onClick={() => setOpen(false)}>{link.name}</a></li>
                    ))}
                </ul>
            </nav>
        </header>
